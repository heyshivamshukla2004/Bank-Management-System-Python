...
""" APNA BANK   — an interactive Streamlit banking app
...
def find_user(data, acc_no, pin):
    matches = [u for u in data if u["account no. "] == acc_no and u["pin"] == pin]
...
# HEADER
# ------------------------------------------------------------------
st.markdown('<div class="glow-title"> APNA BANK </div>', unsafe_allow_html=True)
st.markdown(
...
with st.sidebar:
    ...
    if st.session_state.logged_in:
        ...
        st.markdown(
            f"<p style='text-align:center;color:#eadcff;font-size:0.85rem;'>Acc: {u['account no. ']}</p>",
            unsafe_allow_html=True,
        )
...
# ------------------------------------------------------------------
    ...
    if not st.session_state.logged_in:
        ...
        refresh()
        user = find_user(st.session_state.data, st.session_state.logged_in["account no. "], st.session_state.logged_in["pin"])
...
# ------------------------------------------------------------------
    ...
    if not st.session_state.logged_in:
        ...
        with c2:
            st.write(f"**Account No.:** {user['account no. ']}")
...
# ------------------------------------------------------------------
    ...
    if not st.session_state.logged_in:
        ...
        if st.button("Close Account 💔", use_container_width=True, disabled=not confirm):
            refresh()
            acc_no = st.session_state.logged_in["account no. "]
            st.session_state.data = [u for u in st.session_state.data if u["account no. "] != acc_no]
            save_data(st.session_state.data)
            st.session_state.logged_in = None
